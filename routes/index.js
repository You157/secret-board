...
const Post = require('../models/post');　// DBを設定したﾌｧｲﾙ
const crypto = require('crypto');　// ハッシュ関数を利用するためのモジュール
const oneTimeTokenMap = new Map();　// Key(ﾕｰｻﾞ名)：Value(ﾄｰｸﾝ)とした連想配列
const moment = require('moment-timezone');　// 投稿Dateを見やすく編集する関数
...
router.post('/', (req, res) => {
  ...
  if (oneTimeTokenMap.get(requestedUserName) === requestedOneTimeToken) {
    // req.userとonrTimeTokenの組み合わせが一致する場合
    Post.create({
